...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # ── per-call state ─────────────────────────────────────────────────────
    # In real-world production, the room name generated by SIP contains the caller ID
    # e.g., "jamie-call_+491743024601_CeXJGwGiA75Y"
    # We extract the phone number to perform a real database lookup.
    room_parts = room_name.split("_")
    caller_phone = room_parts[1] if len(room_parts) > 1 and room_parts[1].startswith("+") else None
    
    if caller_phone:
        print(f"  [agent] inbound call from: {caller_phone}", file=sys.stderr)
        # TODO: Replace this with your actual database/API lookup:
        # crm = await fetch_crm_from_db(phone=caller_phone)
        
        # For this hackathon demo, we will check if a JSON file exists for that number
        # If not, we fall back to the default Max Müller profile
        crm = load_crm(caller_phone)
        if not crm:
            crm = load_crm("max_mueller")
    else:
        # Fallback if no phone number in SIP header (e.g. laptop mic test)
    ...
    async def _handle_caller_turn(text: str) -> None:
        ...
        # 6. Dashboard: Jamie transcript (emit BEFORE speaking so they can read along)
